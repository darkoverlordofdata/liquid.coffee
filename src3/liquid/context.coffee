...
class Liquid.Context
  ...
  constructor: (environments = {}, outerScope = {}, registers = {}, rethrowErrors = false, resourceLimits = {}) ->
    ...
    @resourceLimits.renderScoreCurrent or= 0
    @resourceLimits.assignScoreCurrent or= 0
  ...
  resourceLimitsReached: ->
    (@resourceLimits.renderLengthLimit and @resourceLimits.renderLengthCurrent > @resourceLimits.renderLengthLimit) or
    (@resourceLimits.renderScoreLimit  and @resourceLimits.renderScoreCurrent  > @resourceLimits.renderScoreLimit ) or
    (@resourceLimits.assignScoreLimit  and @resourceLimits.assignScoreCurrent  > @resourceLimits.assignScoreLimit )
...
#    filters.forEach (f) ->
  ...
  squashInstanceAssignsWithEnvironments: ->
    last = @scopes.length-1
    for k of @scopes[last]
      for env in environments
        if k in env
          @scopes[last][k] = @lookupAndEvaluate(env, k)
