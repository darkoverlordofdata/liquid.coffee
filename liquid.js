...
var Assign, BlankFileSystem, Block, Capture, Case, Comment, Condition, Context, Cycle, Document, Drop, ElseCondition, For, If, IfChanged, Include, Liquid, Strainer, Tag, Template, Unless, Variable, strftime, _flatten, _ref, _ref1, _ref2, _ref3,
...
Template = (function() {
  Template.fileSystem = new BlankFileSystem();

  Template.tags = {};

  Template.registerTag = function(name, klass) {
    return Template.tags[name] = klass;
  };

  Template.registerFilter = function(filters) {
    return Strainer.globalFilter(filters);
  };
      ...
      if (filterMatches) {
        ...
        filters.forEach(function(f) {
          ...
          if (matches) {
            ...
            filterArgs = [];
            _flatten(f.match(/(?:[:|,]\s*)("[^"]+"|'[^']+'|[^\s,|]+)/g) || []).forEach(function(arg) {
              var cleanupMatch;
...
Capture = (function(_super) {
  ...
  Capture.prototype.render = function(context) {
    ...
    context.set(this.to, _flatten([output]).join(""));
    return "";
  };
...
Case = (function(_super) {
      ...
      while (i < _this.blocks.length) {
        ...
        if (block.isElse) {
          if (execElseBlock === true) {
            output = _flatten([output, _this.renderAll(block.attachment, context)]);
          }
          return output;
  ...
  Case.prototype.recordWhenCondition = function(markup) {
    ...
    while (markup) {
      ...
      markup = parts[2];
      block = new Condition(this.left, "==", parts[1]);
...
Unless = (function(_super) {
  ...
  function Unless() {
    _ref2 = Unless.__super__.constructor.apply(this, arguments);
    return _ref2;
  }
...
IfChanged = (function(_super) {
  ...
  function IfChanged() {
    _ref3 = IfChanged.__super__.constructor.apply(this, arguments);
    return _ref3;
  }
...
Include = (function(_super) {
  ...
  Include.prototype.render = function(context) {
    ...
    return output = _flatten([output]).join("");
  };
